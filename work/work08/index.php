...
<div id="container" class="sub work_gallery">
    <section>
        ...
        $prev_num = $file_num - 1;
        $next_num = $file_num + 1;
        $prev_link =  "work08_".$prev_num;
        $next_link =  "work08_".$next_num;

        $dir = $_SERVER['DOCUMENT_ROOT']."/work/work08/images";
        $scandir = scandir($dir);
        $count = count($scandir);
        $result = strval($count) - 2;
        ?>
        <div class="photo">
            <img src="images/<?=$src?>.jpeg" alt="제이라로비 상세페이지">
        </div><!-- .photo -->
            <div class="btn_prev"><a href="./?id=<?php if($prev_num<10){
                echo "work08_0".$prev_num;
            }else{echo $prev_link;}
                ?>" style="<?php if($prev_num == "0"){
                echo "display:none";} ?>">이전</a></div><!-- .btn_prev -->
            <div class="btn_next"><a href="./?id=<?php if($next_num<10){
                echo "work08_0".$next_num;
